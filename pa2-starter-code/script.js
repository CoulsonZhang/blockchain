...
async function getTotalOwed(user) {
    ...
    for(let otherUser of users){
        if(otherUser !== user){
            let debt = await BlockchainSplitwise.methods.lookup(user, otherUser).call();
...
async function collectTransactions(addressOfContract, functionName) {
